<template>
  <li class="bg-slate-50 border border-slate-100 rounded-lg p-3 space-y-3 list-none">
    <div class="font-semibold text-slate-900">2. 反重力去豆包问完题（你要粘贴两次）</div>
    <p class="text-[11px] text-slate-500 leading-relaxed">
      管理台不会登录豆包。你在反重力里粘贴<strong>两次</strong>：第一次让它按题去问；问完后再粘贴第二次，让它把答案写成 JSON 文件。
    </p>
      ...
      <div class="bg-white border-2 border-[#7c5bf5]/40 rounded-lg p-3 space-y-3">
        <div>
          ...
          <p class="text-[11px] text-slate-600 mt-1 leading-relaxed">
            第一次粘贴<strong>不会</strong>自动落盘。你要再复制一次「收工说明书」贴进<strong>同一个</strong>反重力对话，它才会按路径新建/写入 JSON。写完后，你点「检查有没有落盘」。<strong>注意：</strong>这里查的是「今天这份目标文件名」；若显示今日未写但下方列表仍有历史 JSON、且豆包答案已存进项目，<strong>不等于第 0 步资料丢失</strong>。
          </p>
        </div>
