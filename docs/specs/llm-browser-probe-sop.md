...
- **基线 ≠ Cursor 题单**。创建时只有 `unprobed`；Cursor 出的是剧本；反重力×豆包写出 `competitor_probe_*.json`；管理台第 3 步「确认写入」后才 `baseline_ready`。未回填前，高质量出题提示词必须锁【首轮】，数据源仅创建资料 + 可选期望问题库。
- 探活 = 认知体检：用少量真人长问摸清 AI 认不认识你们、推谁；问句确认后可晋升为**第一波选题**。
...
1. **Cursor / 任意 IDE（出题）**：读 `project.yaml`（含「客户期望完善的问题库」）与既有探针；写出**真人会问的**探活题。工作方式：**先在对话框列题给人改，再落盘** `probe_script_*.json`。管理台「复制高质量出题提示词」。  
2. **管理台阶段 0**：扫描 `outputs/`、预览/确认回填；**模板一键出题**仅断网保底，须改成真人长问。  
3. **反重力（或任意执行 IDE）**：用已登录 Safari 打开豆包，按**剧本文件**提问并落盘结果 JSON（用开工/收工说明书，不是出题提示词）。  
4. **产品**：确认预览中的竞品与问句后回填；合格长问可晋升选题。

人机原则：**题单人味与可讨论性优先于一键速度**；回填由管理台完成；人处理登录态豆包实战与确认。管理台**不**驱动 Safari。
