...
- [ ] 1.1 核对 `AGENTS.md` 生产隔离与 8088 端口规范，锁定 `tools/geo/llm.py` 底座复用、`tools/geo/dist_bot.py` 的 `get_distribution_ledger`、`tools/geo/probing.py` 的 `is_ledger_asset_eligible` 与 `projects/{id}/outputs/factual_anchors.json` 真实档案读取规则（杜绝虚构模块路径；锁定 Query 采样自 `keywords_intent_matrix.json` 的 `flat_queries` 真实字段而非写死特定品牌）。

## 2. 研发 RAG 混合检索与重排演习引擎 (`tools/geo/rerank_simulator.py`)

- [ ] 2.1 构建确定性 RAG 检索沙箱 `RerankSandboxSimulator`（支持构建我方真实切片与竞品干扰切片池，模拟多模型 RAG 上下文重排召回）。
- [ ] 2.2 实现 RAG 混合检索打分模型：Dense 语义相似度 `score_dense_similarity`、Sparse 词频 `score_sparse_bm25` 与 Cross-Encoder 重排算法 `score_cross_encoder_rerank`（公式权重 45% Dense + 35% Sparse + 20% 权威加权）。
- [ ] 2.3 实现重排穿透率算法 `calculate_cpr` 与竞品排挤率算法 `calculate_cor`，完成三档健康等级划分判定。
- [ ] 2.4 实现重排序语义强化包生成器 `generate_rerank_reinforcement_pack`（生成 `outputs/rerank_reinforcement_pack/` 下 3 份强化清单与防御文案），并规范落盘 `outputs/22_跨大模型RAG混合检索召回与重排序挤占演习报告.md`（自适应话术与免责声明）与 `rag_rerank_simulation.json`。
...
- [ ] 5.1 编写 `tests/test_rerank_simulator.py`，全量覆盖：
  ...
  - 断言自适应报告话术（沙箱免责 + 技术推演声明；全 live 包含实盘审计声明）；
