...
5. 若需响应对端 IDE 的修改建议，仅对 `proposal.md`、`design.md` 或 `tasks.md` 进行修订。
6. **【重要阶段边界与停步约束】**：
   - 本技能仅限审查、分析与对齐共识。
   - 完成评审或方案订正后，**必须立即停止任何后续工具调用（STOP）**，向用户简要汇报审查结论，将控制权交还用户。
   - **绝对严禁擅自自动执行编码（opsx-apply）或变更归档（opsx-archive）！**
